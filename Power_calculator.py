...
def calculate_power():
    try:
        ...
        label_result.config(text=(
            f"Active power (P):      {active_power:.2f} W\n"                             #var:.2f -zaokruhli variable na 2 desatinne miesta #✅ 
            f"Apparent power (S): {apparent_power:.2f} VA\n"                             #⚡  
            f"Reactive power (Q):  {reactive_power:.2f} VAr"                             #🔄 
        ))
    except ValueError:
        label_result.config(text="❌ Enter valid numeric values")

# GUI setup
window = Tk()
window.title(f"Power {version}")
...
ttk.Label(window, text="Current (A):", style="My.TLabel").grid(column=0, row=0, sticky="w")            # Label!
...
ttk.Label(window, text="Number of phases:", style="My.TLabel").grid(column=0, row=3, sticky="w")
combo_phase = ttk.Combobox(window, values=["1", "2", "3"], state="readonly", style="My.TCombobox")
combo_phase.set("3")
combo_phase.grid(column=1, row=3)
    
    #Button
ttk.Button(window, text="Calculate power", style="My.TButton", command=calculate_power).grid(column=0, row=4, columnspan=2, pady=10)
...
def show_about():
    ...
    "Aplication: Power calculator\n"
    f"Version: {version}\n\n"
    "Explanations:\n"
    "⚡ What is power factor (cos φ)?\n"
    "- The power factor indicates what portion of the total power is 'active' (useful).\n"
    "- The value ranges between 0 and 1:\n"
    "- 1.0 = ideal state (pure active power, no reactive component)\n"
    "- 0.9 – 0.95 = typical values for modern appliances\n"
    "- 0.8 – 0.9 = older motors, transformers\n"
    "- < 0.8 = highly inductive or capacitive devices\n\n"
...
file_menu.add_command(label="New calculation  Ctrl+N", command=calculate_power)          #Stlacenie vypoctu -funkcia distance
file_menu.add_separator()
file_menu.add_command(label="Quit                       Ctrl+Q", command=window.quit)
menu_bar.add_cascade(label="File", menu=file_menu)

# --- 2 Help/Pomocník ---
help_menu = Menu(menu_bar, tearoff=0)
help_menu.add_command(label="About  F1", command=show_about)          
